...
public class RsyncInvoker {
    ...
    public boolean rsync(String relativePath)
    {
        ...
        try
        {
            ProcessBuilder pb = new ProcessBuilder("rsync", remoteLogDir + "/" + relativePath, localLogDir);
        ...
        backgroundThread = new Thread((Runnable)()->{
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        Thread.sleep(sleep);
                        ProcessBuilder pb = new ProcessBuilder("rsync", "-rvt", remoteLogDir + "/", localLogDir);
                    ...
                    if (exitVal != 0)
                    {
                        ...
                        System.out.println("Failed to rsync retrying in " + sleep / 10000 + " seconds");
                    }
                    else
                    {
                        sleep = 1000;
